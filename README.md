...
| 테스트 | 119개 | 서비스 7개 + 검증기 1개 + provider 1개 + `contextLoads` |
...
| `UserProfileServiceTest` | 15 | 자가 복구 (있음 · 없음 · 탈퇴 표시) · 기본 키 충돌만 다시 읽고 그 밖의 무결성 오류는 그대로 던짐 · 가입 이벤트 3갈래 · 빈 닉네임만 채움 |
...
문제   가입 이벤트가 늦거나 유실되면 계정은 있는데 프로필이 없음
      ...
      화면은 GET /users/me 가 404 면 짧게 몇 번 다시 부르다가 로그아웃시킴
```
...
| 만들다 난 무결성 오류 | 처리 |
|---|---|
| 기본 키 충돌 — SQLState `23505` 이고 제약 이름이 `user_profile_pkey` | 먼저 생긴 행을 다시 읽어 돌려줌 |
| 그 밖 — NOT NULL 위반 · 폭 초과 · 다른 유일 제약 | 받은 예외를 그대로 던짐 → `500 INTERNAL_ERROR` · 오류 로그 |

> 그 밖의 위반은 행이 생기지 않은 실패라 다시 읽어도 행이 없습니다. 이것을 `404` 로 돌려주면 화면이 몇 번
> 다시 부르다 로그아웃시키고, 데이터베이스 오류는 원인이 로그에 남지 않습니다. `500` 이면 화면은 프로필 자리만 비우고
> 공통 예외 처리기가 원래 예외를 오류 로그로 남깁니다.

> 스프링이 던지는 `DataIntegrityViolationException` 바로 밑에 하이버네이트의 제약 위반 예외가 있고,
> 그 예외가 SQLState 와 제약 이름을 담고 있어 둘로 가립니다. 제약 이름은 PostgreSQL 이 표 이름 뒤에
> `_pkey` 를 붙여 만든 기본값이라 표 이름을 바꾸면 함께 바꿔야 합니다.
