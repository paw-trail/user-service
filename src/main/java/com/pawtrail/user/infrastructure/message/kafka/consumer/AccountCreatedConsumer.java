 ...
 * 이 이벤트를 놓쳐도 GET /users/me 가 닉네임 없이 프로필을 만들어 계정은 쓸 수 있으나,
 * 가입 때 입력한 닉네임은 이 이벤트에만 있습니다.
 * DLQ 에 남아 있으면 관리자가 재발행해 그 닉네임을 채울 수 있습니다.
 *
 * 다시 시도에는 기본 키 충돌도 들어옵니다.
 * GET /users/me 가 이 이벤트와 거의 같은 순간에 같은 계정의 프로필을 만들면 한쪽이 충돌로 실패하는데,
 * 이쪽이 실패하면 다시 시도할 때 이미 만들어진 프로필을 보고 비어 있는 닉네임만 채웁니다.
 */
@Slf4j
@Component
